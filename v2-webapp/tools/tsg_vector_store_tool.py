...
def extract_overview_from_tsg(file_path: str) -> str:
    """
    Extract the Overview section from a TSG markdown file
    
    Args:
        file_path: Path to the TSG markdown file
        
    Returns:
        str: Content of the Overview section, or empty string if not found
    """
    try:
        ...
        for i, line in enumerate(lines):
            # Look for "## Overview" header
            if line.strip().lower().startswith('## overview'):
                overview_start = i + 1
            # Look for next ## header after Overview
            elif overview_start != -1 and line.strip().startswith('## ') and not line.strip().lower().startswith('## overview'):
                overview_end = i
                break
        ...
        overview_lines = lines[overview_start:overview_end]
        ...
        print(f"❌ Error extracting overview from {file_path}: {e}")
        return ""
...
async def search_tsg_for_ticket(title: str, summary: str = "", agent_name: str = "TSG Tool") -> dict:
    """
    ...
    Args:
        ...
        agent_name: Name of the agent calling this function for callbacks
    ...
    try:
        if callbacks:
            await callbacks.on_tool_start(agent_name, "TSG Vector Search", {
        ...
        if not search_query:
            if callbacks:
                await callbacks.on_tool_end(agent_name, "TSG Vector Search", "No search query provided")
            return None
        ...
        if not tsg_results:
            ...
            if callbacks:
                await callbacks.on_tool_end(agent_name, "TSG Vector Search", "No TSG results found")
            return None
        ...
        # Extract both overview and solution content
        overview_content = extract_overview_from_tsg(tsg_path) if tsg_path else ""
        ...
        result_message = f"Found best matching TSG: {best_tsg.get('title')} (Similarity: {similarity_percent:.1f}%)"
        print(f"✅ {result_message}")
        
        if callbacks:
            # Send detailed TSG information to frontend
            tsg_details = {
                "tsg_title": best_tsg.get('title'),
                "similarity_percent": f"{similarity_percent:.1f}%",
                "tsg_path": tsg_path,
                "overview": overview_content[:500] + "..." if len(overview_content) > 500 else overview_content,
                "solution": solution_content[:500] + "..." if len(solution_content) > 500 else solution_content,
                "overview_available": bool(overview_content),
                "solution_available": bool(solution_content)
            }
            await callbacks.on_tool_end(agent_name, "TSG Vector Search", tsg_details)
            
            # Also send a formatted message with TSG content for display
#             tsg_display_content = f"""
# 📚 **TSG Match Found**: {best_tsg.get('title')}
# 🎯 **Similarity**: {similarity_percent:.1f}%

# """
#             if overview_content:
#                 tsg_display_content += f"""📖 **Overview**:
# {overview_content}

# """
#             if solution_content:
#                 tsg_display_content += f"""💡 **Solution**:
# {solution_content}

# """
            
#             await callbacks.on_agent_message(agent_name, tsg_display_content.strip())
        
        return best_tsg
            
    except Exception as e:
        error_msg = f"Error searching TSG: {e}"
        print(f"❌ {error_msg}")
        if callbacks:
            await callbacks.on_tool_end(agent_name, "TSG Vector Search", f"Error: {error_msg}")
        return None
...
async def search_tsg_by_text(search_text: str, k: int = 3, agent_name: str = "TSG Tool") -> list:
    """
    ...
    Args:
        ...
        agent_name: Name of the agent for callbacks
        
    Returns:
        list: List of TSG results
    """
    # For testing, we'll just return the single best result as a list
    result = await search_tsg_for_ticket(search_text, "", agent_name)
