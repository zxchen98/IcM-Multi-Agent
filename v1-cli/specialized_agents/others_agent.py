        ...
        if isinstance(msg, AIMessage):
            ...
            if "routing to others_agent" in content:
                if "promptflow supervisor" in content:
                    ...
                    return "prs"
                elif "pipeline supervisor" in content:
                    return "pipeline"
...
def others_agent(state: MessagesState) -> Command[Literal[END]]:
    """
    ...
    if not incident_id:
        ...
        return Command(
            goto=END,
            update={"messages": [AIMessage(content="Others Agent: No incident ID found for analysis.")]}
        )
    ...
    if not incident_details or incident_details.get('title') == 'Unknown':
        ...
        return Command(
            goto=END,
            update={"messages": [AIMessage(content=f"Others Agent: Unable to retrieve details for incident {incident_id}")]}
        )
    ...
    # Combine title and summary for incident description
